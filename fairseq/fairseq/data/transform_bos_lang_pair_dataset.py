...
class TransformBosLangPairDataset(FairseqDataset):
    ...
    def collater(self, samples, **extra_args):
        ...
        if self.new_src_bos is not None:
            if self.dataset.left_pad_source:
                # logger.info(samples["net_input"]["src_tokens"][:, 0])
                # logger.info(self.src_bos)
                # print(samples["net_input"]["src_tokens"][:, 0])
                # assert (
                #     samples["net_input"]["src_tokens"][:, 0] != self.src_bos
                # ).sum() == 0
                samples["net_input"]["src_tokens"][:, 0] = self.new_src_bos
            else:
                bos_idx = samples["net_input"]["src_lengths"] - 1
                assert (
                    samples["net_input"]["src_tokens"][
                        torch.arange(bos_idx.size(0)), bos_idx
                    ]
                    != self.src_bos
                ).sum() == 0
                bos_idx = bos_idx.resize_(len(samples["net_input"]["src_lengths"]), 1)
                samples["net_input"]["src_tokens"].scatter_(
                    1, bos_idx, self.new_src_bos
                )
        ...
        if (
            ...
            if self.dataset.left_pad_target:
                ...
                raise NotImplementedError(
                    "TransformBosLangPairDataset does not implement --left-pad-target True option"
                )
            else:
                assert (
