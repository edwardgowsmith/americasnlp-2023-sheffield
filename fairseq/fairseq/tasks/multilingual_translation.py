...
class MultilingualTranslationTask(LegacyFairseqTask):
    ...
    def prepare(cls, args, **kargs):
        ...
        assert len(paths) > 0
        shared_dict = cls.load_dictionary(
            os.path.join(paths[0], "dict.txt")
        )
        
        if args.encoder_langtok is not None or args.decoder_langtok:
            for lang_to_add in sorted_langs:
                shared_dict.add_symbol(_lang_token(lang_to_add))
        ...
        for name, tok in DATA_SOURCE_PREFIX_TAGS.items():
            shared_dict.add_symbol(tok)

        for lang in sorted_langs:
            dicts[lang] = shared_dict
    ...
    def alter_dataset_langtok(
        ...
        if (
            ...
            and src_bos is not None
            and src_lang is not None
            and tgt_lang is not None
        ):
            new_src_bos = self.get_encoder_langtok(src_lang, tgt_lang)
        else:
            src_bos = None

        new_tgt_bos = None
        if self.args.decoder_langtok and tgt_bos is not None and tgt_lang is not None:
            new_tgt_bos = self.get_decoder_langtok(tgt_lang)
        else:
            tgt_bos = None

        return TransformBosLangPairDataset(
            lang_pair_dataset,
            src_bos=src_bos,
            new_src_bos=new_src_bos,
            tgt_bos=tgt_bos,
    ...
    def load_dataset(self, split, epoch=1, **kwargs):
        ...
        def language_pair_dataset(lang_pair):
            ...
            return self.alter_dataset_langtok(
                langpair_dataset,
                src_bos=self.dicts[src].eos(),
                src_lang=src,
                tgt_bos=self.dicts[tgt].eos(),
                tgt_lang=tgt,
            )
                ...
                [
                    (
                        ...
                        self.alter_dataset_langtok(
                            ...
                            src_bos=self.source_dictionary.eos(),
