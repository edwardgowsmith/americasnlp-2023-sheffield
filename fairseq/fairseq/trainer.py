    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try:
                ...
                # for y in ["encoder.embed_tokens.weight", "decoder.embed_tokens.weight", "decoder.output_projection.weight"]:      
                #     state["model"][y] = torch.cat([state["model"][y], 
                #         torch.nn.init.uniform_(torch.unsqueeze(state["model"][y][-1], 0)),
                #         torch.nn.init.uniform_(torch.unsqueeze(state["model"][y][-1], 0)),
                #         torch.nn.init.uniform_(torch.unsqueeze(state["model"][y][-1], 0)),
                #         torch.nn.init.uniform_(torch.unsqueeze(state["model"][y][-1], 0)),
                #         torch.nn.init.uniform_(torch.unsqueeze(state["model"][y][-1], 0)),
